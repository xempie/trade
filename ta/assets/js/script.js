class TradingForm {
    constructor() {
        ...
        this.pricePrecision = 5; // Default precision, will be set from exchange price
        // Make precision globally accessible
        window.currentPricePrecision = 5;
    ...
    async getBingXPrice(cleanSymbol) {
        try {
            ...
            if (data.success && data.price) {
                // Detect precision from exchange price
                this.pricePrecision = this.detectPricePrecision(data.price);
                window.currentPricePrecision = this.pricePrecision; // Make globally accessible
                console.log(`Price precision detected: ${this.pricePrecision} decimals for ${cleanSymbol}`);
    ...
    // Detect decimal precision from a price string/number
    detectPricePrecision(price) {
    ...
    // Format price using the detected precision
    ...
    calculateEntryPrice(percentFieldId, priceFieldId) {
        ...
        priceEl.value = this.formatPrice(calculatedPrice);
    }
    ...
    async createSignal(data) {
        try {
            ...
            const debugResponse = await fetch('api_proxy.php?endpoint=debug_order_data.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            // Debug the debug response
            const debugResponseText = await debugResponse.text();
            console.log('Raw Debug API Response:', debugResponseText);
            
            let debugResult;
            try {
                debugResult = JSON.parse(debugResponseText);
                console.log('Debug analysis:', debugResult);
            } catch (debugParseError) {
                console.error('Debug JSON Parse Error:', debugParseError);
                console.log('Debug response that failed to parse:', debugResponseText);
                throw new Error(`Invalid JSON response from debug API: ${debugResponseText.substring(0, 200)}...`);
            }
            
            // Call the real order placement API
            const response = await fetch('api_proxy.php?endpoint=place_order.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    displayRecentPositions(positions) {
        ...
        positionList.innerHTML = positions.map(position => {
            ...
            return `
                <div class="signal-item position-item ${isDemo ? 'demo-position' : 'live-position'}" data-position-id="${position.id}">
...
// Global helper function for formatting prices with dynamic precision
window.formatPriceGlobal = function(price, precision = null) {
    const currentPrecision = precision || window.currentPricePrecision || 5;
    return parseFloat(price).toFixed(currentPrecision);
};